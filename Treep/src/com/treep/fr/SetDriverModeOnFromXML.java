...
public class SetDriverModeOnFromXML extends AsyncTask<Void, Integer, HashMap<String, String>> {
	
	private List<NameValuePair> nameValuePairs = new ArrayList<NameValuePair>();
	private HashMap<String,String> mapErrCode = new HashMap<String,String>();
	private Activity activity;
	
	public SetDriverModeOnFromXML(Activity activity){
	...
	protected HashMap<String, String> doInBackground(Void... arg0) {
		...
		String xml = parser.getXmlFromUrl(MainActivity.URL_SETDRIVERMODEON);
	...
	protected void onPostExecute(HashMap<String, String>  result) {
		...
		if(result == null || result.containsKey(MainActivity.KEY_TIMEOUT)){
			...
			if(result.get(MainActivity.KEY_ERRCODE).equals("000")){
	    		...
	    		MainActivity.displayToast("Mode driver activé");
			}
			else{
				MainActivity.displayToast("Problème de connexion. Veuillez réessayer plus tard : "+result.get(MainActivity.KEY_ERRCODE));
			}
			activity.setProgressBarIndeterminateVisibility(false);
		}
	}
